...
async function main() {
  ...
  const chat = fs.readFileSync(path.join(pub, 'client-parts', 'chat-compose.js'), 'utf8');
  const util = fs.readFileSync(path.join(pub, 'client-parts', 'util.js'), 'utf8');
  check('chat 操作片右键菜单含「翻译成中文」', chat.includes('翻译成中文') && chat.includes('translateMessage'));
