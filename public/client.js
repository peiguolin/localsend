(function () {
  ...
  function renderEmojiTabs() {
    ...
    for (const [id, icon, label] of tabs) {
      ...
      b.type = 'button';
      b.className = 'emoji-tab' + (id === emojiActiveCat ? ' active' : '');
      b.textContent = icon;
      b.title = label;
      b.dataset.cat = id;
      b.addEventListener('click', (e) => {
        e.stopPropagation(); // 关键：若重建 DOM，被点掉的按钮脱离面板，会被 document 误判为"点外面"而关闭
        if (emojiActiveCat === id) return;
        emojiActiveCat = id;
        emojiSearchQ = '';
        emojiSearch.value = '';
  ...
  function renderEmojiGrid() {
    ...
    if (emojiSearchQ) {
      ...
      const cat = EMOJI.categories.find(c => c.id === emojiActiveCat);
      const ids = cat ? cat.e : [];
      for (const id of ids) {
        const e = EMOJI.emojis[id];
        if (e && e.c) items.push([id, e]);
    ...
    if (!items.length) {
      ...
      d.textContent = emojiSearchQ ? '没有匹配的表情' : (emojiActiveCat === 'recent' ? '还没有使用过表情' : '该分类暂无表情');
    ...
    const buildChunk = () => {
      ...
      for (; idx < end; idx++) {
        ...
        b.addEventListener('click', (ev) => {
          ...
          if (emojiActiveCat === 'recent') renderEmojiGrid(); // 最近使用：用完立即置顶
        });
        frag.appendChild(b);
      }
  ...
  emojiSearch.addEventListener('input', () => {
    emojiSearchQ = emojiSearch.value.trim();
    renderEmojiGrid();
  });
